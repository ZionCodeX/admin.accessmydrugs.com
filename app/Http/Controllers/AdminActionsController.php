...
class AdminActionsController extends Controller
{
    ...
    */
    ...
    public function admin_action_request_processing_prox(Request $request)
    {
        ...
        //check if modify quote button was clicked
        if($request->buttonx == 'review_quote')
        { 
            //UPDATE REQUEST STATUS
            DB::table('orders')
                    ->where('pid_order', '=', $request->pid_order)
            ...
            if($message == '')
            {
                $message = 'Spreadit Procurement Team have begun reviewing your previously generated quote again. Your record will be modified accordingly, and a quote re-sent.';
            }
        }
            ...
            //MESSAGE META DATA
            //if modify quote is clicked
            if($request->buttonx == 'review_quote')
            {
            //message title
            $message_title = 'Quote Reviewed';
            //message body
			  $message_body = "
							  Dear ".$first_name.",<br>
							  Your quote with ID: <b>".$pid_order."</b> is currently being reviewed.<br><br>
            ...
            ////////////////// EMAIL SENDER STARTS //////////////////
				...
				$xdata = array();
				$xdata['to'] = $data['user']->email;
				$xdata['email_title'] = 'QUOTE REVIEW :: SPREADIT ORDER PROCESSING';
				$xdata['message_title'] = $message_title;
				$xdata['message_body'] = $message_body;
				//$xdata['message_designation'] = "<b>Tochukwu Nkwocha</b><br>Founder / CEO <br>";
				$xdata['from'] = 'admin@spreaditglobal.com';
        ...
        if($request->buttonx == 'create_quote')
        { 
            \Session::flash('success', 'Quote Generation was successfull! Request is now available for Quote Generation.');
            //return view('pages/order_view_index', $data);
        ...
        if($request->buttonx == 'reject')
        { 
            \Session::flash('failed', 'Request was denied available for Quote Generation.');
            //return view('pages/order_view_index', $data);
            return redirect()->route('order_status_view_index', ['status' => $new_status]);
        }
        ...
        if($request->buttonx == 'review_quote')
        { 
            \Session::flash('success', 'Quote has been moved back to processing stage for your review.');
            //return view('pages/order_view_index', $data);
            return redirect()->route('order_status_view_index', ['status' => $new_status]);
        }
   ...
   //INVOICE REVIEW ACTION
   public function admin_action_review_expired_invoice_prox(Request $request)
   {
       ...
       //check if lock request button was clicked
       if($request->buttonx == 'review_invoice')
       { 
           //UPDATE REQUEST STATUS
           DB::table('orders')
                   ->where('pid_order', '=', $request->pid_order)
           ...
           {
           //message title
           $message_title = 'Reviewed Invoice Processing';
           //message body
             $message_body = "
                             Dear ".$first_name.",<br>
                             Your expired Invoice with ID: <b>".$pid_order."</b> is currently being reviewed.<br><br>
                             Our Quote Processing Team should get back to you any time soon.<br><br>
                             Please check your Email or Spreadit-Procurement Dashboard frequently.<br><br>
                             We may call or send an email if we need more information.<br><br>
                             <b>::ADMIN ADDITIONAL INFO::</b><br>"
           ...
           ////////////////// EMAIL SENDER STARTS //////////////////
               ...
               $xdata = array();
               $xdata['to'] = $data['user']->email;
               $xdata['email_title'] = 'EXPIRED INVOICE REVIEW :: SPREADIT ORDER PROCESSING';
               $xdata['message_title'] = $message_title;
               $xdata['message_body'] = $message_body;
               //$xdata['message_designation'] = "<b>Tochukwu Nkwocha</b><br>Founder / CEO <br>";
               $xdata['from'] = 'admin@spreaditglobal.com';
       ...
       //////////////////// REQUIRED CORE DATA ////////////////////
           ...
           \Session::flash('success', 'Expired Invoice has been moved back to processing stage for your review.');
           //return view('pages/order_view_index', $data);
           return redirect()->route('order_status_view_index', ['status' => $new_status]);
        ...
        { 
            ...
            //$data['user'] = XTR::count(['users','id','=',2]);
            //////////////////// REQUIRED CORE DATA ////////////////////
            return view('pages/order_status_view_index', $data);exit;
    ...
    public function admin_action_quote_generated_prox(Request $request)
    {
        ...
        $total = DB::table('products')->where('pid_order','=',$pid_order)->sum(DB::raw('product_unit_price * product_quantity'));
        //$data['user'] = XRecordsController::user('load_user_all');
        //$data['count_users_registered'] = XTR::count(['users']);
        //$data['count_users_activated'] = DB::table('users')->whereNotNull('email_verified_at')->where('xstatus', '=', 1)->count();
        //$data['count_users_unactivated'] = DB::table('users')->whereNull('email_verified_at')->where('xstatus', '=', 1)->count();
    ...
    //QUOTE MODIFY QUOTE PROCESSING ACTION
    public function admin_action_modify_quote_processing_prox(Request $request)
    {
   ...
   public function admin_action_order_processing_prox(Request $request)
   {
       ...
       $user_records = DB::table('users')
           ...
           DB::table('messages')->insert(
             ...
             $message_body = "
                             Dear ".$first_name.",<br>
                             Your Order with ID: <b>".$pid_order."</b> is currently being processed for shipping.<br><br>
                             You may track your Order Process from your dashboard.<br><br>
                             Please check your Email or Spreadit-Procurement Dashboard frequently.<br><br>
                             We may call or send an email if we need more information.<br><br>
                             <b>::ADMIN ADDITIONAL INFO::</b><br>"
                             .$message.
           ...
           ////////////////// EMAIL SENDER STOPS //////////////////     
            ...
            //////////////////// REQUIRED CORE DATA ////////////////////
                ...
                \Session::flash('success', 'Order Successfully moved to Processing Stage.');
                //return view('pages/order_view_index', $data);
                return redirect()->route('order_status_view_index', ['status' => $new_status]);
