...
@section('title', 'Create NewsFeed Post')
    ...
    <div class="card card-custom card-sticky" id="kt_page_sticky_card">
        <div class="card-header" style="">
          ...
          <div class="card-title">
            <h3 class="card-label">Create NewsFeed Post
            <i class="mr-2"></i>
            <small class=""> Create an News / Article</small></h3>
          </div>
...
<!--##### XIS STANDARD FORM #####-->
<form method="post" action="{{ route('post_create_form_prox'); }}"  enctype="multipart/form-data">
    @csrf
...
@component('form.textarea')
    ...
    @slot('value') @endslot
    @slot('placeholder') Enter Article Tags for SEO here separated by commas @endslot
    @slot('hint') For SEO Optimization provide tags separated by commas e.g. house, rent, esatate, to-let, sell-property, land etc. (Optional) @endslot
    @slot('maxlength') 300 @endslot
    @slot('rows') 3 @endslot
    @slot('required') @endslot
@endcomponent
...
@component('form.select')
    ...
    @slot('required') required @endslot
        @slot('options')
            ...
            <option value="PRODUCT_FINDER"> PRODUCT FINDER </option>
...
@component('form.upload')
    ...
    @slot('label') Upload Post Image @endslot
...
@component('form.button')
    ...
    @slot('label') Create Post @endslot
